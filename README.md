...
- **Archivos** (logos, portadas, comprobantes) con almacenamiento configurable: `local` (con Volume de Railway), `cloudinary` o `s3`.
...
Cada cliente = un proyecto de Railway con **2 cosas**: el servicio de la app y una base Postgres.

1. **Crea un proyecto** en Railway y agrega el plugin **PostgreSQL**.
2. **Agrega un servicio** desde este repositorio (o un fork/copia por cliente). El `railway.json` de la raíz ya define build y arranque:
   - Build: `npm install && npm run build`
   - Start: migra la base → corre el seed (idempotente) → arranca la API (que sirve el frontend).
3. **Variables del servicio** (pestaña Variables):
   ...
   - `COOKIE_SECRET` → otro secreto aleatorio de 32+ caracteres
   - Opcionales: `ADMIN_USER`, `ADMIN_PASSWORD`, `SITE_NAME` (nombre inicial de la página)
4. **Volume** para las imágenes: agrega un Volume al servicio montado en `/data` (el API guarda en `/data/uploads`).
5. **Dominio**: genera el dominio público (o conecta el dominio del cliente, ej. `rifadeejemplo.com`).

Listo: la raíz del dominio muestra la página del rifero y `tudominio.com/admin` pide iniciar sesión.
